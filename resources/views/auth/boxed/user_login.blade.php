                    ...
                    <div class="card shadow-none rounded-0 border-0">
                        ...
                        <div class="row no-gutters" style="margin: 5px">
                            <div class="col-lg-6" style="text-align: center">
                                <a href="{{route("seller.login")}}">{{ translate('Seller Login Page Image') }}</a>
                            </div>
                            <div class="col-lg-6" style="text-align: center">
                                <a href="{{route("user.login")}}">{{ translate('Customer Login Page Image') }}</a>
                            </div>
                        </div>
                        -->
                                        ...
                                        <form class="form-default" role="form" action="{{ route('login') }}" method="POST">
                                            ...
                                            @if (addon_is_activated('otp_system'))
                                                ...
                                                <div class="form-group email-form-group mb-1 d-none">
                                                    <label for="email" class="fs-12 fw-700 text-soft-dark">{{ "يمكنك تسحيل الدخول باستخدام الايميل او رقم الهاتف او اسم المستخدم او رقم العضوية" }}</label>
                                                    <input type="text" class="form-control rounded-0 {{ $errors->has('email') ? ' is-invalid' : '' }}" value="{{ old('email') }}" placeholder="{{  translate('johndoe@example.com') }}" name="input" id="email" autocomplete="off">
                                                    @if ($errors->has('email'))
                                            ...
                                            @else
                                                <div class="form-group">
                                                    <label for="email" class="fs-12 fw-700 text-soft-dark">{{ "يمكنك تسحيل الدخول باستخدام الايميل او رقم الهاتف او اسم المستخدم او رقم العضوية" }}</label>
                                                <input type="text" class="form-control rounded-0 {{ $errors->has('email') ? ' is-invalid' : '' }}" value="{{ old('email') }}" placeholder="{{  translate('johndoe@example.com') }}" name="login" id="email" autocomplete="off">
                                                    @if ($errors->has('email'))
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $errors->first('email') }}</strong>
                                                        </span>

                    ...
                    <form action="{{ route('payment.checkout') }}" class="form-default" role="form" method="POST"
                        ...
                        <div class="card rounded-0 border shadow-none">
                            @if( $carts && count($carts) > 0 )
                                <?php
                                $tickets = \App\Models\Ticket::where('user_id', Auth::user()->id)->get();
                                ?>
                                                    ...
                                                    @foreach ($carts as $key => $cartItem)
                                                        ...
                                                        <li class="list-group-item px-0">
                                                            <div class="row gutters-5 align-items-center">
                                                                <input type="hidden" product_name="{{ $product->getTranslation('name')  }}" id="id_{{$key+1}}" value="{{$product->id}}">
                                                                <!-- Product Image & name -->
                                                                ...
                                                                <div class="col-md-1 col order-1 order-md-0">
                                                                    @if ($cartItem['digital'] != 1 && $product->auction_product == 0)
                                                                        <div class="d-flex flex-column align-items-start aiz-plus-minus mr-2 ml-0">
                                                                            ...
                                                                            <input type="number" readonly name="quantity[{{ $cartItem['id'] }}]"
                            ...
                            @endif
                                ...
                                <div class="form-group px-4" style="text-align: center;display: none">
                                    <textarea style="margin-bottom: 10px"  id="jsonOutput" name="additional_info" rows="5" class="form-control rounded-0"
                                              placeholder="{{ translate('Type your text...') }}"></textarea>
                                    <button type="button" onclick="generateJSON()" id="generateJsonBtn" class="btn btn-primary fs-14 fw-700 rounded-0 px-4">{{ translate('Install')}}</button>
                                    <script>
                                        ...
                                        function generateJSON() {
                                            ...
                                            const customers = getAllNames();

                                            customers.forEach(customerName => {
                                                const products = document.querySelectorAll('[id^="id_"]');
                                                ...
                                                products.forEach(product => {
                                                    ...
                                                    const productName = product.getAttribute('product_name');
                                                    const count = getFromAll(customerName,productIndex);
                                        ...
                                        function getFromAll(customerName,productIndex) {

                                            let count = 0;
                                            const selects = document.querySelectorAll('[id^="name_'+productIndex+'_"]');
                                            selects.forEach(select => {
                                                if (select.value == customerName) {
                                                      ++count;
                                                }
                                            });
                                            return count;
                                        ...
                                        function getAllNames() {
                                            const select = document.getElementById('name_1_1');
                                            const valuesArray = Array.from(select.options).map(option => option.value);
                                            return valuesArray;
                                        }
                            ...
                            <div class="row align-items-center pt-3 px-4 mb-4">
                                ...
                                <div class="col-6 text-right">
                                    <button type="button" onclick="generateJSON();submitOrder(this)"

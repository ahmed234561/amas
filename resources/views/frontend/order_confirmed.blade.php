                    ...
                    <div class="text-center py-4 mb-0">
                        ...
                        <p class="fs-13 text-soft-dark">
                            ...
                            <strong>
                                @php
                                    $shipping_email = '';
                                    if(isset($first_order->shipping_address)) {
                                        $shipping_address = json_decode($first_order->shipping_address, true);
                                        $shipping_email = $shipping_address['email'] ?? ($first_order->user->email ?? 'N/A');
                                    }
                                    echo $shipping_email;
                                @endphp
                            </strong>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border-top-0 py-2">
                                            @php
                                                ...
                                                if(isset($first_order->shipping_address)) {
                                                    $shipping_address = json_decode($first_order->shipping_address, true);
                                                    $name = $shipping_address['name'] ?? ($first_order->user->name ?? 'N/A');
                                                }
                                                echo $name;
                                            @endphp
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border-top-0 py-2">
                                            @php
                                                ...
                                                if(isset($first_order->shipping_address)) {
                                                    $shipping_address = json_decode($first_order->shipping_address, true);
                                                    $email = $shipping_address['email'] ?? ($first_order->user->email ?? 'N/A');
                                                }
                                                echo $email;
                                            @endphp
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border-top-0 py-2">
                                            @php
                                                $shipping_address = '';
                                                if(isset($first_order->shipping_address)) {
                                                    $address_data = json_decode($first_order->shipping_address, true);
                                                    $shipping_address = ($address_data['address'] ?? '') . ', ' .
                                                                      ($address_data['city'] ?? '') . ', ' .
                                                                      ($address_data['country'] ?? '');
                                                }
                                                echo $shipping_address ?: 'N/A';
                                            @endphp
                                        </td>
                                    </tr>
                                </table>
                                                    ...
                                                    <tr>
                                                            ...
                                                            @if ($orderDetail->product != null)
                                                                <a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank" class="text-reset">
                                                                    ...
                                                                    @if($orderDetail->combo_id != null)
                                                                        @php
                                                                            $combo = \App\ComboProduct::find($orderDetail->combo_id);
                                                                        @endphp
                                                                        @if($combo)
                                                                            ({{ $combo->combo_title }})
                                                                        @endif
                                                        ...
                                                        <td class="border-top-0 border-bottom">
                                                            ...
                                                            @elseif ($order->shipping_type == 'carrier')
                                                                {{ translate('Carrier') }}
                                                            @elseif ($order->shipping_type == 'pickup_point' && $order->pickup_point)

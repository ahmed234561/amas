    ...
    public function login(Request $request)
{
    ...
    // البحث عن المستخدم باستخدام البريد الإلكتروني، الرقم، أو الرمز البريدي
    $user = User::where('email', $login)
                ->orWhere('phone', $login)
                ->orWhere('name', $login)
                ->orWhere('postal_code', $login)
                ->first();

    // التحقق من وجود المستخدم
    if ($user && Auth::attempt(['id' => $user->id, 'password' => $password])) {
        // تسجيل الدخول بنجاح
 //       return redirect()->intended('dashboard'); // تغيير 'dashboard' إلى المسار المناسب
        // if (session('temp_user_id') != null) {
        //     Cart::where('temp_user_id', session('temp_user_id'))
        //         ->update(
        //             [
        ...
        if (auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'staff') {
            ...
            return redirect()->route('seller.dashboard');
        } else if(auth()->user()->postal_code != null) {
                return redirect()->route('dashboard');
        }
        else {

...
class ClubPointController extends Controller
{
    ...
    public function processClubPoints(Order $order)
    {
        ...
        $club_point->points = 0;
        $club_point->points_type = $order->points_type;
        if($order->points_type == 'saudian_points') {
            foreach ($order->orderDetails as $key => $orderDetail) {
                $total_pts = ($orderDetail->earn_point) * $orderDetail->quantity;
                $club_point->points += $total_pts;
            }
        } else if($order->points_type == 'malaysian_points') {
            foreach ($order->orderDetails as $key => $orderDetail) {
                $total_pts = ($orderDetail->malaysian_points) * $orderDetail->quantity;
                $club_point->points += $total_pts;
            }
        }
        ...
        if($club_point->points > 0){
            ...
            foreach ($order->orderDetails as $key => $orderDetail) {
                ...
                if($order->points_type == 'malaysian_points') {
                    $club_point_detail->point = ($orderDetail->malaysian_points) * $orderDetail->quantity;
                }
		...
		if($club_point->convert_status == 0) {
            ...
            foreach ($club_point->club_point_details as $club_point_detail) {
                if($club_point_detail->refunded == 0){
                    $club_point_detail->converted_amount = 0;//floatval($club_point_detail->point / get_setting('club_point_convert_rate'));
                    $club_point_detail->save();
